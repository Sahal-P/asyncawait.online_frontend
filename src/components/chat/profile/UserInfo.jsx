...
const UserInfo = () => {
  ...
  const imgUrl = `${
    selected?.contact?.profile?.profile_picture
      ? selected?.contact?.profile?.profile_picture
      : import.meta.env.VITE_S3_MEDIA_URL + selected?.contact?.profile?.default_avatar
  }`;
  return (
    <div className="mt-[60px] w-full ">
      <div className="w-full h-[315px] bg-[#111B21] flex flex-col items-center justify-center">
        ...
        <Image
          ...
          width={200}
          height={315}
          hash={selected?.contact?.profile?.picture_blurhash}
        />
        </div>
        <h1 className="text-white text-2xl">{selected?.contact?.profile?.username ?? selected?.contact?.phone_number}</h1>
        <p className="text-slate-500">{selected?.contact?.phone_number}</p>
      </div>

      <div className="w-full h-[88px] bg-secondary mt-3 px-8 flex flex-col justify-center">
        <h3 className="text-slate-500">About</h3>
        <p className="text-white">{selected?.contact?.profile?.about}</p>
      </div>
      ...
      <div className="w-full h-[454px] bg-secondary mt-3 pl-10 pr-8 flex flex-col gap-3">
        ...
        <div className="flex items-center w-full h-[50px] mt-3">
          <div className="flex text-red-400 gap-5">
            ...
            <div>
              <h1 className="text-red-400">{`Block ${selected?.contact?.profile?.username ?? selected?.contact?.phone_number}`}</h1>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center w-full h-[50px] mt-3">
          <div className="flex text-red-400 gap-5">
            ...
            <div>
              <h1 className="text-red-400">{`Report ${selected?.contact?.profile?.username ?? selected?.contact?.phone_number}`}</h1>
            </div>
          </div>
        </div>
